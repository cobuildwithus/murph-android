...
- “Synced” is rendered only from `GET /api/device-sync/companion/status?sourceProviderSlug=health_connect`. Unresolved per-resource foreground failures accumulate across retries and cannot be hidden by a later local complete pass or another category's newer source-wide receipt. Each known failure clears only after its own backend receipt advances past the conservative failure floor, or after current permission truth proves that category was revoked. If the failure marker cannot be committed, setup is durably forced through reconnect instead of projecting success after reconstruction.
