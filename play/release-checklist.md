...
- [ ] Complete the Health Apps declaration and every Health Connect data-type justification from `declarations/health-apps.md`; confirm the exact candidate still has eleven configured resources covering eleven data-type permissions and requests neither extended-history nor background access.
...
- [ ] Demonstrate Health Connect setup, selective grants, initial data receipt, permission loss, reconnect, sign-out, and member switch on supported Pixel and Samsung devices.
