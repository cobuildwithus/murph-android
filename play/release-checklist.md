...
- [ ] On API 34–36 Release devices, preserve one existing active grant across two failed post-permission setup attempts, then retry without changing the system selection until Vital reports `NotPrompted`. Confirm setup can recover from that current grant alone; verify detail-only dependency guidance remains specific and cancellation, unavailability, or unknown permission failure cannot advance setup.
...
- [ ] On API 31–36, block the starter immediately before foreground promotion, press Home, and release it. Confirm no child resource read or Junction receipt starts and foreground return owns the ordinary retry. Then allow promotion and one real child reader to begin, press Home, and confirm the starter and every child drain without another read. Kill the process during that interruption and prove recreation requires explicit reconnect, does not restart Vital work headlessly, and cannot become Synced from local completion or a partial receipt. Complete reconnect and prove only a strictly newer source-scoped backend receipt makes Synced eligible.
...
- [ ] On the same device matrix, hold a real resource reader/uploader open after WorkManager reports its request cancelled. Independently trigger sign-out, member switch, and Delete Account; confirm Junction sign-out, replacement identification, browser handoff, and local reset remain blocked until the delegated worker body exits, with no post-boundary receipt.
- [ ] Force an early authorized resource to fail while later independent resources have valid records. Confirm every later resource is attempted in priority order and can produce a receipt, the overall sync reports partial failure, and cancellation or lease revocation still prevents the next child from starting.
...
- [ ] Demonstrate the optional sync reminder with notification permission allowed and denied, opt-in and opt-out, foreground cancellation, cold-process notification delivery without application-graph or vendor initialization, notification delivery into Settings, sign-out, and member switch on the same device matrix.
