...
5. **Backend membership before health work.** A current, verified Privy member must pass the read-only member/consent status check before setup or every app-triggered health sync. Offline restoration always returns through this check when Privy becomes online-verified, with or without an existing health setup.
...
13. **Process-owned transitions.** `AppGraph` owns the application-lifetime coroutine scope used for session, login, sync, and sign-out work. Activity recreation can replace the renderer without cancelling those transitions.
