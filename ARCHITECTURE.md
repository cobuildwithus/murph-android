...
12. **Reviewed health scope.** The resource set is centralized in `JunctionHealthSyncService`, mirrored by 29 manifest permissions, and checked against all 21 `VitalResource` values in the pinned SDK. Vital 5.0.2 globally discovers granted resources, but the adapter holds its public synchronization pause across permission and connect flows and unpauses only while sending the configured-and-granted intersection to explicit foreground sync. An empty intersection remains a no-op. Aggregate activation still follows SDK dependencies: Exercise is required for workout elevation, power, and speed, while Menstruation is required for the four reproductive detail types. The complete permission surface is not an end-to-end ingestion claim; source-app export and backend receipt for each granted family remain explicit physical-device release gates.
