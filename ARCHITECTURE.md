...
8. **Trust-boundary teardown.** Account switching, sign-out, backend rejection, and a signed-in Junction SDK without a completed setup marker clear the local Junction session before more health work. Sign-out synchronously removes the durable setup authorization before either SDK call, so interruption can never authorize a later `resume`.
